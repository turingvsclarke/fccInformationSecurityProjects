...
const {reportReply,reportThread,deleteReply,deleteThread,insertReply,insertThread,getThread,getThreadsFromBoard}=require('../database.js');
...
module.exports = function (app) {
  ...
  app.route('/api/replies/:board').get((req,res)=>{
    getThread(req.query.thread_id).then((thread)=>{
      res.send(thread)
    })
  ...
  */
  app.route('/api/threads/:board').delete((req,res)=>{
    deleteThread(req.body.thread_id,req.body.delete_password).then((validation)=>{
      if(validation){
        res.send('success')
      }else{
  ...
  **/
  app.route('/api/replies/:board').delete((req,res)=>{
    deleteReply(req.body.reply_id,req.body.delete_password).then((validation)=>{
      if(validation){
        res.send('success')
      }else{
        res.send('incorrect password')
  ...
  ***/
  app.route('/api/threads/:board').put((req,res)=>{
    reportThread(req.body.thread_id).then(()=>{
      res.send('reported')
    })
  })
  ...
  ***/
  app.route('/api/replies/:board').put((req,res)=>{
    reportThread(req.body.reply_id).then(()=>{
      res.send('reported')
    })
  })
};
